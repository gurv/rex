...
mod cache_engine {
    ...
    fn creates_cache_dir_tag() {
        ...
        CacheEngine::new(sandbox.path().join(".rex")).unwrap();

        assert!(sandbox.path().join(".rex/cache/CACHEDIR.TAG").exists());
    }

    #[test]
    fn returns_default_if_cache_missing() {
        let sandbox = create_empty_sandbox();
        let engine = CacheEngine::new(sandbox.path().join(".rex")).unwrap();
        let item = engine
            .state
    ...
    fn can_write_cache_if_mode_off() {
        ...
        assert!(sandbox.path().join(".rex/cache/test.json").exists());
    ...
    fn can_write_cache_if_mode_readonly() {
        ...
        assert!(sandbox.path().join(".rex/cache/states/test.json").exists());
