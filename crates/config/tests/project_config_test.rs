...
fn load_config_from_root(root: &Path, source: &str) -> miette::Result<ProjectConfig> {
    ConfigLoader::new(root.join(".rex")).load_project_config_from_source(root, source)
}
...
mod project_config {
    ...
    fn error_unknown_field() {
        test_load_config("rex.yml", "unknown: 123", |path| {
    ...
    #[test]
    fn loads_defaults() {
        let config = test_load_config("rex.yml", "{}", |path| load_config_from_root(path, "."));
...
",
    ...
    mod depends_on {
        ...
        fn supports_list_of_strings() {
            let config = test_load_config("rex.yml", "dependsOn: ['a', 'b', 'c']", |path| {
                load_config_from_root(path, ".")
            });
...
",
    ...
    mod language {
        ...
        #[test]
        fn supports_variant() {
            let config = test_load_config("rex.yml", "language: rust", |path| {
                load_config_from_root(path, ".")
            });
        ...
        fn unsupported_variant_becomes_other() {
            let config = test_load_config("rex.yml", "language: groovy", |path| {
                load_config_from_root(path, ".")
            });
    ...
    mod owners {
        ...
        #[test]
        fn loads_defaults() {
            let config = test_load_config("rex.yml", "owners: {}", |path| {
                load_config_from_root(path, ".")
            });
...
",
        ...
        fn errors_on_invalid_format() {
            test_load_config("rex.yml", "tags: ['foo bar']", |path| {
                load_config_from_root(path, ".")
            });
        }
    }
