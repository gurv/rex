...
use rex_config::{ConfigLoader, ToolchainsConfig};
...
const FILENAME: &str = ".rex/toolchains.yml";
...
fn load_config_from_root(root: &Path, proto: &ProtoConfig) -> miette::Result<ToolchainsConfig> {
    ConfigLoader::new(root.join(".rex")).load_toolchains_config(root, proto)
}
...
"
        ...
        fn loads_from_url_and_saves_temp_file() {
            ...
            sandbox.create_file(".rex/toolchains.yml", format!(r"extends: '{url}'"));
