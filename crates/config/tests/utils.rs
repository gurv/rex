...
pub fn load_project_config_in_format(format: &str) {
    ...
    let config = test_config(locate_fixture(format), |path| {
        ConfigLoader::new(path.join(".rex")).load_project_config(path)
    });
...
pub fn load_tasks_config_in_format(format: &str) {
    ...
    let config = test_config(locate_fixture(format), |path| {
        ConfigLoader::new(path.join(".rex"))
            .load_tasks_config_from_path(path, path.join(format!(".rex/tasks.{format}")))
    });
...
pub fn load_template_config_in_format(format: &str) {
    ...
    let config = test_config(locate_fixture(format), |path| {
        ConfigLoader::new(path.join(".rex")).load_template_config(path)
    });
...
pub fn load_toolchains_config_in_format(format: &str) {
    ...
    let config = test_config(&fixture, |path| {
        ...
        ConfigLoader::new(path.join(".rex")).load_toolchains_config(path, &proto)
    });
    ...
    assert_eq!(
        ...
        &ToolchainPluginConfig {
            plugin: Some(PluginLocator::File(Box::new(FileLocator {
                ...
                path: Some(fixture.join(".rex").join("node.wasm")),
            }))),
...
pub fn load_extensions_config_in_format(format: &str) {
    ...
    let fixture = locate_fixture(format);
    let config = test_config(&fixture, |path| {
        ConfigLoader::new(path.join(".rex")).load_extensions_config(path)
    });
...
pub fn load_workspace_config_in_format(format: &str) {
    ...
    let config = test_config(locate_fixture(format), |path| {
        ConfigLoader::new(path.join(".rex")).load_workspace_config(path)
    });
    ...
    assert_eq!(
        ...
        VcsConfig {
            ...
            hooks: FxHashMap::from_iter([(
                "pre-commit".into(),
                vec![
                    "rex check --all --affected".into(),
                    "rex run :pre-commit".into()
                ]
            )]),
