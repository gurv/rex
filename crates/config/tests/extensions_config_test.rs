...
use rex_config::{ConfigLoader, ExtensionPluginConfig, ExtensionsConfig};
use proto_core::warpgate::{PluginLocator, UrlLocator};
use rustc_hash::FxHashMap;
use std::path::Path;
use utils::*;

const FILENAME: &str = ".rex/extensions.yml";

fn load_config_from_root(root: &Path) -> miette::Result<ExtensionsConfig> {
    ConfigLoader::new(root.join(".rex")).load_extensions_config(root)
}
