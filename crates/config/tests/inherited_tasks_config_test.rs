...
fn load_config_from_file(path: &Path) -> miette::Result<InheritedTasksConfig> {
    ConfigLoader::new(path.join(".rex")).load_tasks_config_from_path(path.parent().unwrap(), path)
}

fn load_manager_from_root(root: &Path, rex_dir: &Path) -> miette::Result<InheritedTasksManager> {
    ConfigLoader::new(rex_dir).load_tasks_manager_from(root, rex_dir)
}
...
"
        ...
        fn loads_from_url_and_saves_temp_file() {
            ...
            let temp_dir = sandbox.path().join(".rex/cache/temp");
            let url = server.url("/config.yml");
            ...
            test_config(sandbox.path().join("tasks/all.yml"), |path| {
                let config = ConfigLoader::new(sandbox.path().join(".rex"))
