...
use rex_pdk_api::VirtualPath;
use rex_warpgate_pdk::{AnyResult};
use serde::de::DeserializeOwned;

/// Get workspace-level configuration for the current subcommand plugin.
pub fn get_subcommand_config<T: Default + DeserializeOwned>() -> AnyResult<T> {
    let config: T = if let Some(value) = config::get("rex_subcommand_config")? {
        json::from_str(&value)?
    } else {
        T::default()
    };
...
/// Parse workspace & project merged configuration for the current subcommand plugin.
pub fn parse_subcommand_config<T: Default + DeserializeOwned>(value: json::Value) -> AnyResult<T> {
...
pub fn parse_subcommand_config_schema<T: schematic::Config>(value: json::Value) -> AnyResult<T> {
