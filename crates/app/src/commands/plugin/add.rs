...
use rex_common::Id;
use rex_config::{ExtensionsConfig, PartialExtensionPluginConfig};
use rex_console::ui::{Container, Notice, StyledText, Variant};
use rex_extension_plugin::{ExtensionPlugin, ExtensionRegistry};
use rex_pdk_api::InitializeExtensionInput;
...
pub struct PluginAddArgs {
    #[arg(help = "Unique ID of the plugin to add")]
    id: Id,

    #[arg(help = "Plugin locator string to find and load")]
...
pub async fn add(session: RexSession, args: PluginAddArgs) -> AppResult {
    ...
    session.console.render(element! {
        Container {
            Notice(variant: Variant::Success) {
                StyledText(
                    content: format!(
                        "Added plugin <id>{}</id> to <file>{}</file>!",
                        extension.id,
