...
use rex_warpgate::{FileLocator, GitHubLocator, Id, PluginLoader, PluginLocator, UrlLocator};
    ...
    mod source_url {
        ...
        async fn errors_broken_url() {
            ...
            loader
                .load_plugin(
                    Id::raw("test"),
                    PluginLocator::Url(Box::new(UrlLocator { url: "https://github.com/gurv/deno-plugin/releases/download/v0.0.2/deno_plugin_invalid_name.wasm".into() })),
                )
                .await
                .unwrap();
        }
        ...
        async fn downloads_to_plugins() {
            ...
            let path = loader
                .load_plugin(
                    Id::raw("test"),
                    PluginLocator::Url(Box::new(UrlLocator { url: "https://github.com/gurv/deno-plugin/releases/download/v0.0.2/deno_plugin.wasm".into() })),
                )
                .await
                .unwrap();
        ...
        async fn supports_latest() {
            ...
            let path = loader
                .load_plugin(
                    Id::raw("test"),
                    PluginLocator::Url(Box::new(UrlLocator { url: "https://github.com/gurv/deno-plugin/releases/latest/download/deno_plugin.wasm".into() })),
                )
                .await
                .unwrap();
            ...
            loader
                .load_plugin(
                    ...
                    PluginLocator::GitHub(Box::new(GitHubLocator {
                        repo_slug: "gurv/invalid-repo".into(),
                        tag: None,
                        project_name: None,
                    })),
                )
