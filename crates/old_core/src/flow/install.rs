use super::build::*;
pub use super::build_error::RexBuildError;
pub use super::install_error::RexInstallError;
use crate::checksum::*;
use crate::env::RexConsole;
...
/// Installs a tool into rex's store.
...
impl<'tool> Installer<'tool> {
    ...
    /// Install a tool into rex, either by downloading and unpacking
    /// a pre-built archive, or by using a native installation method.
    #[instrument(skip(self, options))]
    pub async fn install(
        &self,
        options: InstallOptions,
    ) -> Result<Option<LockRecord>, RexInstallError> {
        ...
        if is_offline() {
            return Err(RexInstallError::RequiredInternetConnection);
        }
            ...
            .await
        {
            ...
            if !output.skip_install {
                return Err(RexInstallError::FailedInstall {
    ...
    /// into the `~/.rex/tools/<version>` folder.
    #[instrument(skip(self, options))]
    async fn build_from_source(
        &self,
        options: InstallOptions,
    ) -> Result<LockRecord, RexInstallError> {
        debug!(
            tool = self.tool.context.as_str(),
        ...
        if !self
            ...
            .await
        {
            return Err(RexInstallError::UnsupportedBuildFromSource {
                tool: self.tool.get_name().to_owned(),
            });
        }
        ...
        let config = self.tool.rex.load_config()?;
        ...
        let mut builder = Builder::new(BuilderOptions {
            ...
            install_dir: &self.product_dir,
            http_client: self.tool.rex.get_plugin_loader()?.get_http_client()?,
            log_writer: options
                .log_writer
                .as_ref()
        ...
        // so allow rex to use any available version instead of failing
        ...
        // Step 4
        execute_instructions(&mut builder, &output, &self.tool.rex).await?;

        Ok(record)
    }

    /// Download the tool (as an archive) from its distribution registry
    /// into the `~/.rex/tools/<version>` folder, and optionally verify checksums.
    #[instrument(skip(self, options))]
    async fn install_from_prebuilt(
        ...
        if !self
            ...
            .await
        {
            return Err(RexInstallError::UnsupportedDownloadPrebuilt {
                tool: self.tool.get_name().to_owned(),
            });
        }

        let client = self.tool.rex.get_plugin_loader()?.get_http_client()?;
        let config = self.tool.rex.load_config()?;
    ...
    #[instrument(skip_all)]
    pub async fn uninstall(&self) -> Result<bool, RexInstallError> {
            ...
            .await
        {
            ...
            if !output.uninstalled && !output.skip_uninstall {
                return Err(RexInstallError::FailedUninstall {
                    tool: self.tool.get_name().to_owned(),
                    error: output.error.unwrap_or_default(),
                });
            }
    ...
    pub async fn verify_checksum(
    ...
    ) -> Result<Checksum, RexInstallError> {
        debug!(
            tool = self.tool.context.as_str(),
            download_file = ?download_file,
        ...
        Err(RexInstallError::InvalidChecksum {
