pub use super::manage_error::RexManageError;
use crate::cfg;
use crate::config::{PinLocation, RexConfig};
use crate::flow::install::{InstallOptions, Installer, RexInstallError};
...
use rex_pdk_api::{PluginFunction, SyncManifestInput, SyncManifestOutput};
...
impl<'tool> Manager<'tool> {
    ...
    pub async fn install(
    ...
    ) -> Result<Option<LockRecord>, RexManageError> {
        ...
        RexConfig::update_document(self.tool.rex.get_config_dir(PinLocation::Global), |doc| {
    ...
    async fn post_install(&self, spec: &mut ToolSpec, force: bool) -> Result<(), RexManageError> {
        // Link all the things
    ...
    #[instrument(skip_all)]
    pub async fn uninstall(&mut self, spec: &mut ToolSpec) -> Result<bool, RexManageError> {
        self.cleanup().await?;
        ...
        RexConfig::update_document(rex.get_config_dir(PinLocation::Global), |doc| {
            if doc
    ...
    #[instrument(skip_all)]
    pub async fn cleanup(&self) -> Result<(), RexManageError> {
        debug!(
        ...
        fs::remove_dir_all(self.tool.get_temp_dir()).map_err(|error| {
            RexManageError::Install(Box::new(RexInstallError::Fs(Box::new(error))))
        })?;
    ...
    #[instrument(skip_all)]
    pub async fn sync_manifest(self) -> Result<(), RexManageError> {
        if !self
            .tool
            .plugin
