pub use super::locate_error::RexLocateError;
use crate::helpers::{ENV_VAR, get_exe_file_name};
...
// Binary/shim = File within rex's store directories
...
impl<'tool> Locator<'tool> {
    ...
    pub async fn locate(
        tool: &'tool Tool,
        spec: &'tool ToolSpec,
    ) -> Result<LocatorResponse, RexLocateError> {
        Self::new(tool, spec).locate_all().await
    }

    /// Locate all applicable executable and global paths.
    pub async fn locate_all(&mut self) -> Result<LocatorResponse, RexLocateError> {
        Ok(LocatorResponse {
    ...
    pub(crate) async fn call_locate_executables(
        &self,
    ) -> Result<LocateExecutablesOutput, RexLocateError> {
        Ok(self
            .tool
            .plugin
            .cache_func_with(
    ...
    pub async fn locate_primary_exe(&self) -> Result<Option<ExecutableLocation>, RexLocateError> {
    ...
    pub async fn locate_secondary_exes(&self) -> Result<Vec<ExecutableLocation>, RexLocateError> {
        let output = self.call_locate_executables().await?;
    ...
    /// Return a list of all binaries that get created in `~/.rex/bin`.
    ...
    ) -> Result<Vec<ExecutableLocation>, RexLocateError> {
    ...
    #[instrument(skip_all)]
    pub async fn locate_exe_file(&mut self) -> Result<PathBuf, RexLocateError> {
        ...
        Err(RexLocateError::MissingToolExecutable {
    ...
    #[instrument(skip_all)]
    pub async fn locate_exes_dirs(&mut self) -> Result<Vec<PathBuf>, RexLocateError> {
    ...
    #[instrument(skip_all)]
    pub async fn locate_globals_dir(&mut self) -> Result<Option<PathBuf>, RexLocateError> {
    ...
    #[instrument(skip_all)]
    pub async fn locate_globals_dirs(&mut self) -> Result<Vec<PathBuf>, RexLocateError> {
        ...
        'outer: for dir_lookup in output.globals_lookup_dirs {
            ...
            for cap in ENV_VAR.captures_iter(&dir_lookup) {
                ...
                let replace_with = match find_by {
                    "$CWD" | "$PWD" => self.tool.rex.working_dir.clone(),
                    "$HOME" | "$USERHOME" | "$USERPROFILE" => self.tool.rex.home_dir.clone(),
                    "$REX_HOME" | "$REX_ROOT" => self.tool.rex.store.dir.clone(),
    ...
    #[instrument(skip_all)]
    pub async fn locate_globals_prefix(&mut self) -> Result<Option<String>, RexLocateError> {
