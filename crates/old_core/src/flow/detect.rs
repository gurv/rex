pub use super::detect_error::RexDetectError;
use crate::config::{DetectStrategy, REX_CONFIG_NAME};
use crate::file_manager::RexConfigFile;
use crate::tool::Tool;
...
impl<'tool> Detector<'tool> {
    ...
    pub async fn detect(
        tool: &'tool Tool,
    ) -> Result<(ToolSpec, Option<PathBuf>), RexDetectError> {
    ...
    pub async fn detect_version(&mut self) -> Result<ToolSpec, RexDetectError> {
            ...
            && !session_version.is_empty()
        {
            ...
            return Ok(UnresolvedVersionSpec::parse(&session_version)
                .map_err(|error| RexDetectError::InvalidDetectedVersionSpec {
        ...
        trace!(
            tool = self.tool.context.as_str(),
            "Attempting to find version from {} files", REX_CONFIG_NAME
        );

        let config_files = self.tool.rex.load_config_files()?;
        ...
        let detected_version = match config.settings.detect_strategy {
            ...
            DetectStrategy::PreferRextools => {
                self.detect_version_prefer_prototools(&config_files).await?
            }
            DetectStrategy::OnlyRextools => {
        ...
        Err(RexDetectError::FailedVersionDetect {
    ...
    pub async fn detect_version_from(
        &mut self,
        current_dir: &Path,
    ) -> Result<Option<(UnresolvedVersionSpec, PathBuf)>, RexDetectError> {
        if !self
            .tool
            .plugin
        ...
        for file in output.files {
            ...
            let version = if has_parser {
                ...
                UnresolvedVersionSpec::parse(&content).map_err(|error| {
                    RexDetectError::InvalidDetectedVersionSpec {
                        error: Box::new(error),
    ...
    pub async fn detect_version_first_available(
        &mut self,
        config_files: &[&RexConfigFile],
    ) -> Result<Option<UnresolvedVersionSpec>, RexDetectError> {
        for file in config_files {
            if let Some(version) = self.detect_from_rex_config(file)? {
    ...
    fn detect_from_rex_config(
            ...
            && let Some(version) = versions.get(&self.tool.context)
        {
            debug!(
                ...
                version = version.to_string(),
                file = ?file.path,
                "Detected version from {} file", REX_CONFIG_NAME
            );
