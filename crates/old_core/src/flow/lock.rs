pub use super::lock_error::RexLockError;
...
impl<'tool> Locker<'tool> {
    ...
    pub fn insert_record_into_lockfile(&self, record: &LockRecord) -> Result<(), RexLockError> {
        ...
        let Some(mut lock) = rex.load_lock_mut()? else {
            return Ok(());
        };
            ...
            .find(|existing| existing.is_match(&record, &self.tool.metadata.lock_options))
        {
            Some(existing) => {
                ...
                if self.tool.metadata.lock_options.ignore_os_arch {
                    ...
                    existing.os.get_or_insert(rex.os);
                    existing.arch.get_or_insert(rex.arch);
                }
            }
            None => {
                records.push(record);
    ...
    pub fn remove_from_lockfile(&self) -> Result<(), RexLockError> {
        let Some(mut lock) = self.tool.rex.load_lock_mut()? else {
            return Ok(());
        };
    ...
    pub fn resolve_locked_record(
        &self,
        spec: &ToolSpec,
    ) -> Result<Option<LockRecord>, RexLockError> {
        let rex = &self.tool.rex;

        let Some(lock) = rex.load_lock()? else {
            return Ok(None);
        };
    ...
    pub fn verify_locked_record(
        ...
        let make_error = |actual: String, expected: String| match &install_record.source {
            Some(source) => RexLockError::MismatchedChecksumWithSource {
                checksum: actual,
                lockfile_checksum: expected,
                source_url: source.to_owned(),
            },
            None => RexLockError::MismatchedChecksum {
                checksum: actual,
                lockfile_checksum: expected,
            },
        };
        ...
        if let Some(l_os) = install_record.os
            ...
            && l_os != r_os
        {
            return Err(RexLockError::MismatchedOs {
                os: l_os.to_string(),
        ...
        if let Some(l_arch) = install_record.arch
            ...
            && l_arch != r_arch
        {
            return Err(RexLockError::MismatchedArch {
