...
use crate::env::{RexConsole, RexEnvironment};
use crate::helpers::extract_filename_from_url;
use crate::id::Id;
use crate::lockfile::LockRecord;
use crate::utils::log::LogWriter;
use crate::utils::process::{self, ProcessResult, RexProcessError};
...
impl Builder<'_> {
    ...
    pub async fn prompt_continue(&self, label: &str) -> Result<(), RexBuildError> {
        ...
        if !confirmed {
            return Err(RexBuildError::Cancelled);
        }
    ...
    pub async fn exec_command(
        ...
        command: &mut Command,
        piped: bool,
    ) -> Result<Arc<ProcessResult>, RexProcessError> {
    ...
    fn handle_process_result(
        ...
        if result.exit_code > 0 {
            return Err(RexProcessError::FailedCommandNonZeroExit {
...
async fn get_command_version(
    ...
    Version::parse(value).map_err(|error| RexBuildError::FailedVersionParse {
...
pub async fn check_requirements(
    ...
    if builder.has_errors() {
        return Err(RexBuildError::RequirementsNotMet);
    }
    ...
    for (index, instruction) in build.instructions.iter().enumerate() {
        ...
        match instruction {
            BuildInstruction::InstallBuilder(item) => {
                ...
                let builder_dir = rex.store.builders_dir.join(item.id.as_str());
                ...
                for (exe_name, exe_rel_path) in exes {
                    ...
                    if !exe_abs_path.exists() {
                        return Err(RexBuildError::MissingBuilderExe {
                            exe: exe_abs_path,
                            id: item.id.clone(),
                        });
                    }
