pub use super::resolve_error::RexResolveError;
...
impl<'tool> Resolver<'tool> {
    ...
    pub async fn resolve(
        ...
        spec: &mut ToolSpec,
        short_circuit: bool,
    ) -> Result<VersionSpec, RexResolveError> {
    ...
    pub async fn load_versions(
        ...
        if !cached {
            if is_offline() {
                return Err(RexResolveError::RequiredInternetConnectionForVersion {
                    command: format!(
                        "{}_VERSION=1.2.3 {}",
                        self.tool.get_env_var_prefix(),
                        self.tool.get_id()
                    ),
                    bin_dir: self.tool.rex.store.bin_dir.clone(),
                });
            }

            if env::var("REX_BYPASS_VERSION_CHECK").is_err() {
                versions = self
                    .tool
                    .plugin
                    .cache_func_with(
    ...
    pub async fn resolve_version_candidate(
        ...
        let resolve = |candidate: &UnresolvedVersionSpec| {
            ...
            result.ok_or_else(|| RexResolveError::FailedVersionResolve {
