...
use rex_pdk_api::{Checksum, ChecksumAlgorithm};
...
) -> Result<Checksum, RexChecksumError> {
    match detect_checksum_algorithm(checksum_file)? {
        ChecksumAlgorithm::Minisign => Ok(Checksum::minisign(
            checksum_public_key
                .ok_or(RexChecksumError::MissingPublicKey)?
                .to_owned(),
        )),
        ChecksumAlgorithm::Sha512 => Ok(Checksum::sha512(
            hash_file_contents_sha512(download_file).map_err(|error| RexChecksumError::Sha {
                error: Box::new(error),
            })?,
        )),
        _ => Ok(Checksum::sha256(
            hash_file_contents_sha256(download_file).map_err(|error| RexChecksumError::Sha {
                error: Box::new(error),
            })?,
        )),
    }
...
pub fn detect_checksum_algorithm(
    checksum_file: &Path,
) -> Result<ChecksumAlgorithm, RexChecksumError> {
            ...
            if line.contains(':') {
                    ...
                    && label.trim() == "Algorithm"
                {
                    algo = match value.trim() {
                        ...
                        other => {
                            return Err(RexChecksumError::UnsupportedAlgorithm {
                                algo: other.into(),
                            });
                        }
                    };
    ...
    algo.ok_or_else(|| RexChecksumError::UnknownAlgorithm {
