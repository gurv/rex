...
pub enum RexProcessError {
    #[diagnostic(transparent)]
    #[error(transparent)]
    Fs(#[from] Box<FsError>),

    #[diagnostic(code(rex::process::command_failed))]
...
impl From<FsError> for RexProcessError {
    fn from(e: FsError) -> RexProcessError {
        RexProcessError::Fs(Box::new(e))
    }
}
...
pub async fn exec_command(command: &mut Command) -> Result<ProcessResult, RexProcessError> {
    ...
    let output =
        spawn_command(command)
            .await
            .map_err(|error| RexProcessError::FailedCommand {
...
pub async fn exec_command_piped(command: &mut Command) -> Result<ProcessResult, RexProcessError> {
...
) -> Result<ProcessResult, RexProcessError> {
...
pub fn handle_exec(result: ProcessResult) -> Result<ProcessResult, RexProcessError> {
