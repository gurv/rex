...
use rex_pdk_api::{LoadVersionsOutput, ToolInventoryOptions};
...
impl Inventory {
    ...
    pub fn load_remote_versions(
        &self,
        disable_cache: bool,
    ) -> Result<Option<LoadVersionsOutput>, RexLayoutError> {
        let cache_path = self
            .dir_original
            .as_ref()
            .unwrap_or(&self.dir)
    ...
    #[instrument(skip_all)]
    pub fn save_remote_versions(&self, data: &LoadVersionsOutput) -> Result<(), RexLayoutError> {
        json::write_file(
            self.dir_original
                .as_ref()
                .unwrap_or(&self.dir)
