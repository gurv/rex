use crate::id::Id;
use rex_pdk_api::{Checksum, ToolLockOptions};
...
pub const REX_LOCK_NAME: &str = ".rexlock";
...
impl RexLock {
    ...
    pub fn load<P: AsRef<Path> + Debug>(path: P) -> Result<Self, TomlError> {
        ...
        let mut manifest: RexLock = if path.exists() {
            toml::read_file(path)?
        } else {
            RexLock::default()
        };
    ...
    pub fn save(&self) -> Result<(), TomlError> {
        ...
        fs::write_file(
            &self.path,
            format!("# Generated by rex. Do not modify!\n\n{content}"),
        )?;
    ...
    fn resolve_path(path: impl AsRef<Path>) -> PathBuf {
        ...
        if path.ends_with(REX_LOCK_NAME) {
            path.to_path_buf()
        } else {
            path.join(REX_LOCK_NAME)
        }
    }
}
