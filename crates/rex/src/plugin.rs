...
//!
//! This module provides functions for installing, uninstalling, and listing rex plugins.
//! Plugins are WebAssembly components that extend rex functionality.
...
use crate::{
    ...
    runtime::{
        ...
        bindings::{
            ...
            plugin::{
                RexPlugin,
                exports::vg::rex::plugin::{HookType, Metadata},
            },
        },
        plugin::Runner,
...
/// implements the rex plugin interface. It contains the component itself, its metadata,
...
impl PluginComponent {
    pub async fn new(
        ...
        // the component is a valid rex plugin.
        let plugin = crate::runtime::bindings::plugin::RexPlugin::new(&mut store, &instance)
            .context("failed to create plugin host bindings")?;
        let metadata = plugin.vg_rex_plugin().call_info(&mut store).await?;
    ...
    pub async fn call_info(&self, ctx: Ctx) -> anyhow::Result<Metadata> {
        ...
        let plugin = crate::runtime::bindings::plugin::RexPlugin::new(&mut store, &instance)
            .context("failed to create plugin host bindings")?;
        plugin.vg_rex_plugin().call_info(&mut store).await
    }
    ...
    pub async fn call_hook(
        ...
        let instance = self
            ...
            .await
            .context("failed to instantiate plugin")?;
        let plugin = RexPlugin::new(&mut store, &instance)
            .context("failed to create plugin host bindings")?;
    ...
    pub async fn call_run(
        &self,
        ctx: Ctx,
        command: &bindings::plugin::vg::rex::types::Command,
        runner_context: Arc<RwLock<HashMap<String, String>>>,
    ) -> anyhow::Result<String> {
        let mut store = self.component.new_store(ctx);
        let instance = self
...
/// A struct responsible for managing Rex plugins
#[derive(Debug)]
...
pub async fn install_plugin(
    ...
    // Validate that it's a valid WebAssembly component and rex plugin
...
/// Built-in rex commands that cannot be overridden by plugins
...
fn validate_plugin_commands(metadata: &Metadata) -> anyhow::Result<()> {
    ...
    if let Some(ref command) = metadata.command {
        ...
        if BUILT_IN_COMMANDS.contains(&command_name.as_str()) {
            bail!(
                "Plugin command '{}' conflicts with built-in rex command. \
    ...
    if BUILT_IN_COMMANDS.contains(&plugin_name.as_str()) {
        bail!(
            "Plugin name '{}' conflicts with built-in rex command. \
            Built-in commands that cannot be overridden: {}",
            metadata.name,
            BUILT_IN_COMMANDS.join(", ")
        );
...
mod tests {
    use super::*;
    use crate::runtime::bindings::plugin::vg::rex::types::{Command, HookType};
    ...
    fn test_validate_plugin_commands_rejects_conflicting_command_name() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("conflicts with built-in rex command")
        );
    }
