//! WIT dependency management for rex components
//!
...
//     /// The path to the cache directory. Defaults to the rex cache directory.
...
//         // 2. Path provided by the user via `REX` prefixed environment variable
//         // 3. Path provided by the users via `WKG` prefixed environment variable
//         // 4. Default path to config file in rex dir
//         // 5. Default path to config file from wkg
//         match (self.config.as_ref(), std::env::var_os("WKG_CONFIG_FILE")) {
//             // We have a config file provided by the user flag or REX env var
//             (Some(path), _) => {
...
//         // 2. Path provided by the user via `REX` prefixed environment variable
//         // 3. Path provided by the users via `WKG` prefixed environment variable
//         // 4. Default path to cache in rex dir
//         let dir = match (self.cache.as_ref(), std::env::var_os("WKG_CACHE_DIR")) {
//             // We have a cache dir provided by the user flag or REX env var
...
// ///
// /// The lock file is expected to be at `wkg.lock` in the project root, or `.rex/wasmcloud.lock` relative to the project directory.
// #[instrument(skip_all)]
...
//     let project_lock_file_path = project_dir.join(".rex").join(LOCK_FILE_NAME);
...
//         debug!(path = %project_lock_file_path.display(), "found .rex/wasmcloud.lock file, using it");
...
//         // Ensure the .rex directory exists for when we create the file
//         let rex_dir = project_dir.join(".rex");
//         tokio::fs::create_dir_all(&rex_dir)
//             .await
//             .with_context(|| {
//                 format!(
//                     "failed to create .rex directory: {rex_dir}",
//                     rex_dir = rex_dir.display()
//                 )
//             })?;
