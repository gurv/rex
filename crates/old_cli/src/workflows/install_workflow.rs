...
use crate::session::RexConsole;
use crate::shell::{self, Export};
use crate::telemetry::*;
use crate::utils::tool_record::ToolRecord;
use iocraft::element;
use rex_old_core::flow::install::{InstallOptions, InstallPhase};
use rex_old_core::flow::link::Linker;
use rex_old_core::flow::manage::Manager;
use rex_old_core::flow::resolve::Resolver;
use rex_old_core::utils::log::LogWriter;
use rex_old_core::{Id, LockRecord, PinLocation, ToolSpec};
use rex_pdk_api::{
...
impl InstallWorkflow {
    pub fn new(tool: ToolRecord, console: RexConsole) -> Self {
    ...
    ) -> Result<InstallOutcome, RexCliError> {
    ...
    pub async fn install_with_logging(
        ...
        let log_path = self
            ...
            .rex
            .working_dir
            .join(format!("rex-{}-install.log", self.tool.get_id()));
    ...
    async fn pre_install(
        ...
        unsafe { env::set_var("REX_INSTALL", tool.get_id()) };
    ...
    async fn do_install(
        ...
        spec: &mut ToolSpec,
        params: &InstallWorkflowParams,
    ) -> Result<Option<LockRecord>, RexCliError> {
        params.log_writer.as_ref().inspect(|log| {
    ...
    async fn pin_version(
        ...
        // via `--pin` arg
        if let Some(_custom_type) = arg_pin_to {
            pin = true;
        }
    ...
    async fn update_shell(
        ...
        let profile_path = tool.rex.store.load_preferred_profile()?.and_then(|file| {
            if file.exists() {
                Some(file)
            } else {
                debug!(
...
impl InstallWorkflowManager {
    pub fn new(console: RexConsole, quiet: bool) -> Self {
        Self {
            console,
