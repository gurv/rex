...
use rex_old_core::{Id, REX_PLUGIN_KEY, SemVer, ToolContext, UnresolvedVersionSpec, is_offline};
use semver::Version;
use serde::Serialize;
...
pub async fn upgrade(session: RexSession, args: UpgradeArgs) -> AppResult {
    if is_offline() {
        return Err(RexCliError::UpgradeRequiresInternet.into());
    }
        ...
        session.console.render(element! {
            Notice(variant: Variant::Info) {
                StyledText(content: if target_version == current_version {
                    format!("You're already on version <version>{current}</version> of rex!")
                } else if has_explicit_target {
                    format!("An explicit version of rex will be used: {target_chain}")
                } else if target_version > current_version {
                    format!("A newer version of rex is available: {target_chain}")
                } else {
                    format!("An older version of rex is available: {target_chain}")
                })
            }
        })?;
    ...
    if not_available {
        session.console.render(element! {
            Notice(variant: Variant::Info) {
                StyledText(
                    content: format!("You're already on version <version>{current}</version> of rex!")
                )
            }
        })?;
    ...
    if let Some(pid) = is_running() {
        session.console.render_err(element! {
            Notice(variant: Variant::Caution) {
                StyledText(
                    content: format!("Another instance of <shell>rex</shell> is currently running with the process ID {}. You may run into issues if you continue.", pid)
                )
            }
        })?;
    ...
    install_one(
        ...
        ToolContext::new(Id::raw(REX_PLUGIN_KEY)),
    )
    .await?;

    // Replace the global binaries
    let tool_dir = session.env.store.inventory_dir.join(REX_PLUGIN_KEY);
    ...
    let upgraded = replace_binaries(
        ...
        !is_current_exe_managed && env::var("REX_TEST").is_err(),
    )?;
            ...
            Notice(variant: Variant::Success) {
                StyledText(
                    content: if target_version >= current_version {
                        format!("Upgraded rex to <version>{target}</version>!")
                    } else {
                        format!("Downgraded rex to <version>{target}</version>!")
                    }
                )
            }
        })?;
...
fn is_running() -> Option<sysinfo::Pid> {
    ...
    debug!(
        self_pid = self_pid,
        "Checking if rex is currently running in a separate process"
    );

    let mut system = sysinfo::System::new();
    system.refresh_processes(ProcessesToUpdate::All, true);

    for process in system.processes_by_name("rex".as_ref()) {
...
) -> Result<bool, RexCliError> {
    let source_dir = source_dir.as_ref();
    let target_dir = target_dir.as_ref();
    let bin_names = vec![path::exe_name("rex")];
