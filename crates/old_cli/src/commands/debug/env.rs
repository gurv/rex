...
use clap::Args;
use iocraft::prelude::*;
use rex_old_core::layout::Store;
use rex_pdk_api::{HostArch, HostOS};
use serde::Serialize;
...
pub async fn env(session: RexSession, args: DebugEnvArgs) -> AppResult {
    let env = &session.env;
    ...
    let environment = EnvironmentInfo {
        ...
        configs: manager
            ...
            .collect(),
        os: HostOS::from_env(),
        rex_version: env!("CARGO_PKG_VERSION").into(),
        vars: env::vars()
            .filter_map(|(k, v)| {
                if k.starts_with("REX_") {
                    Some((k, v))
                } else {
                    None
                }
    ...
    session.console.render(element! {
        Container {
            ...
            Section(title: "Environment") {
                Entry(
                    name: "Rex version",
                    content: environment.rex_version,
                )
                Entry(
