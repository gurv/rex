...
use crate::error::RexCliError;
use crate::session::RexSession;
use crate::workflows::{ExecWorkflow, ExecWorkflowParams};
use clap::Args;
use miette::IntoDiagnostic;
use rex_old_core::flow::detect::{Detector, RexDetectError};
use rex_old_core::flow::locate::{Locator, RexLocateError};
use rex_old_core::flow::resolve::Resolver;
use rex_old_core::layout::ShimRegistry;
use rex_old_core::{
    Id, REX_PLUGIN_KEY, RexEnvironment, RexLoaderError, Tool, ToolContext, ToolSpec,
};
use rex_pdk_api::ExecutableConfig;
use rex_shim::{exec_command_and_replace, locate_rex_exe};
...
use rex_system_env::create_process_command;
use tracing::debug;
...
fn should_use_global_rex(tool: &Tool) -> miette::Result<bool> {
    if tool.get_id() != REX_PLUGIN_KEY {
        return Ok(false);
    }

    let config = tool.rex.load_config()?;
    let rex_context = ToolContext::new(Id::raw(REX_PLUGIN_KEY));

    Ok(
        // No pinnned version
        !config.versions.contains_key(&rex_context)
        // Pinned but the same as the running process
        || config.versions.get(&rex_context).is_some_and(|v| v.req.to_string() == env!("CARGO_PKG_VERSION")),
    )
}

fn should_hide_auto_install_output(args: &[String]) -> bool {
    envx::bool_var("REX_AUTO_INSTALL_HIDE_OUTPUT")
...
async fn get_tool_executable(
    ...
    if let Some(alt_name) = alt {
        ...
        return Err(RexCliError::RunMissingAltBin {
    ...
    let mut config = match locator.locate_primary_exe().await? {
        ...
        None => {
            return Err(RexLocateError::NoPrimaryExecutable {
...
fn get_global_executable(env: &RexEnvironment, name: &str) -> Option<PathBuf> {
    ...
    for path_dir in env::split_paths(&system_path) {
        ...
        // Local development may have ~/.rex on PATH, so ignore!
        #[cfg(debug_assertions)]
        if path_dir.to_string_lossy().contains(".rex") {
            continue;
        }
...
// of rex, so try and fallback to it!
fn run_global_tool(
    session: RexSession,
    args: RunArgs,
    error: miette::Report,
    ...
    if let Some(global_exe) = get_global_executable(&session.env, args.context.id.as_str()) {
        debug!(
            ...
            "Tool {} is currently not managed by rex but exists on PATH, falling back to the global executable",
...
pub async fn run(session: RexSession, mut args: RunArgs) -> AppResult {
    let tool = match session.load_tool(&args.context).await {
        Ok(tool) => tool,
        Err(RexLoaderError::UnknownTool { id }) => {
            ...
            // which is maintained by rex during tool installation.
            debug!(
                bin = id.as_str(),
            ...
            if let Some(parent_id) = parent_tool_id {
                ...
                return run_global_tool(session, args, RexLoaderError::UnknownTool { id }.into())
                    .map(|_| None);
            }
        }
        Err(error) => {
            return if matches!(error, RexLoaderError::UnknownTool { .. }) {
    ...
    let mut use_global_rex = should_use_global_rex(&tool)?;

    // Avoid running the tool's native self-upgrade as it conflicts with rex
    if is_trying_to_self_upgrade(&tool, &args.passthrough) {
        return Err(RexCliError::RunNoSelfUpgrade {
            command: format!("rex install {} latest --pin", tool.context),
    ...
    let (mut spec, detected_source) = if use_global_rex {
        (
            args.spec
                .clone()
        ...
        match Detector::detect(&tool).await {
            ...
            Err(error) => {
                return if matches!(error, RexDetectError::FailedVersionDetect { .. }) {
                    run_global_tool(session, args, error.into()).map(|_| None)
                } else {
                    Err(error.into())
                };
    ...
    if tool.is_installed(&spec) {
        ...
        // If this is the rex tool running, continue instead of failing
        else if use_global_rex {
            debug!(
                "No rex version detected or located, falling back to the global rex executable!"
            );
        }
        // Otherwise fail with a not installed error
        else {
            let command = format!("rex install {} {}", tool.context, resolved_version);

            if let Some(source) = detected_source {
                return Err(RexCliError::RunMissingToolWithSource {
                    tool: tool.get_name().to_owned(),
            ...
            return Err(RexCliError::RunMissingTool {
                tool: tool.get_name().to_owned(),
                version: spec.req.to_string(),
                command,
            }
    ...
    let exe_config = if use_global_rex {
        ExecutableConfig {
            exe_path: locate_rex_exe("rex"),
            primary: true,
            ..Default::default()
        }
    } else {
    ...
    workflow
        .prepare_environment(
            ...
            ExecWorkflowParams {
                ...
                pre_run_hook: true,
                version_env_vars: !use_global_rex,
                ..Default::default()
            },
        )
        .await?;
