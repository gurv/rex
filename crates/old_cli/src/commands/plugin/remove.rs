...
use rex_old_core::{Id, REX_CONFIG_NAME, PinLocation, PluginType, RexConfig};
...
pub struct RemovePluginArgs {
    ...
    #[arg(long, default_value_t, help = "Location of .rextools to remove from")]
    from: PinLocation,
...
pub async fn remove(session: RexSession, args: RemovePluginArgs) -> AppResult {
    let config_dir = session.env.get_config_dir(args.from);
    let config_path = config_dir.join(REX_CONFIG_NAME);

    if !config_path.exists() {
        return Err(RexCliError::MissingToolsConfigInCwd { path: config_path }.into());
    }

    let config_path = RexConfig::update_document(config_dir, |doc| {
