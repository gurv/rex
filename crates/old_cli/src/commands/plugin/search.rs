...
use rex_old_core::PluginLocator;
use rex_old_core::registry::PluginFormat;
...
pub async fn search(session: RexSession, args: SearchPluginArgs) -> AppResult {
    ...
    session.console.render(element! {
        Container {
            View(padding_top: 1, padding_left: 1, flex_direction: FlexDirection::Column) {
                ...
                StyledText(
                    content: "Learn more about plugins: <url>https://moonrepo.dev/docs/rex/plugins</url>"
                )
            }
            Table(
                headers: vec![
            ...
            View(padding_bottom: 1, padding_left: 1, flex_direction: FlexDirection::Row) {
                ...
                StyledText(
                    content: "rex plugin add [id] [locator]",
                    style: Style::Shell
                )
            }
        }
