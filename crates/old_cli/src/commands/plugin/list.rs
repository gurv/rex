...
use crate::session::{LoadToolOptions, RexSession};
use clap::Args;
use iocraft::prelude::element;
use rex_old_core::{ConfigMode, Id, PluginLocator, RexToolConfig, ToolContext, ToolManifest};
...
pub async fn list(session: RexSession, args: ListPluginsArgs) -> AppResult {
    ...
    let mut tools = session
        .load_tools_with_options(LoadToolOptions {
            contexts: FxHashSet::from_iter(if args.ids.is_empty() {
    ...
    for tool in tools {
        ...
        let is_tool = !tool.is_backend_plugin();
