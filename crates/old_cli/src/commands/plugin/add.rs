...
use rex_old_core::{Id, PinLocation, PluginLocator, PluginType, RexConfig, cfg};
...
pub struct AddPluginArgs {
    ...
    #[arg(long, default_value_t, help = "Location of .rextools to add to")]
    to: PinLocation,
...
pub async fn add(session: RexSession, args: AddPluginArgs) -> AppResult {
    let config_path = RexConfig::update_document(session.env.get_config_dir(args.to), |doc| {
    ...
    {
        use rex_old_core::ToolContext;

        let tool = rex_old_core::load_tool_from_locator(
